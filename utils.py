...
def get_func_info(file_name): #获取函数名及分数和各自的测试用例
    ...
    for line in all_lines:
        ...
        sz = len(tokens)
        if sz == 5: #tokens[0]函数名；tokens[1]分数；tokens[2]总分；tokens[3]测试文件
            all_func_info.append((tokens[0], float(tokens[1]), float(tokens[2]), tokens[3], int(tokens[4]))) # + '.test_cases.txt'})
        else:
...
def empty_file_info(file_name): #新增 清空文件内容
    f = open(file_name,'w')
    f.truncate()
    f.close()
